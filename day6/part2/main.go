...
func computeArea(region Region, threshold int) int {
	plots := region.plots
	area := 0

	for _, row := range plots {
		for _, plot := range row {
			if plot.distance < int32(threshold) {
				area++
			}
		}
	}
...
func main() {
	args := os.Args
	if len(args) < 3 {
		fmt.Println("Please provide a threshold and the path to the input as the first argument")
		os.Exit(1)
	}
	...
	largestArea := computeArea(region, threshold)
